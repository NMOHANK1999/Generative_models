...
class DiffusionModel(nn.Module):
    def __init__(
        ...
        #alphas_back = torch.cat((torch.tensor([1], device = self.device), alphas[:-1]), dim = 0)
        #self.alphas_cumprod_prev = torch.cumprod(alphas_back, dim = 0).to(self.device)     
    ...
    def model_predictions(self, x_t, t):
        ...
        coeff_xt = extract(self.x_0_pred_coef_1, t, x_t.shape)
        coeff_pred_noise = extract(self.x_0_pred_coef_2, t, pred_noise.shape)
        
        x_0 = coeff_xt * x_t - coeff_pred_noise * pred_noise
        
        #see the image predicted here
        #the pred noise is only one step noise
    ...
    def ddim_step(self, batch, device, tau_i, tau_isub1, img, model_predictions, alphas_cumprod, eta):
        ...
        t = torch.full((batch,), tau_i, device = self.betas.device, dtype = torch.long)
        ...
        sigma_tau_i_sq = eta * beta_tau_i

        # Step 4: Compute the coefficient of \epsilon_{\tau_{i}}
        coef_epsilon_tau_i = torch.sqrt(1. - alpha_tau_isub1 - sigma_tau_i_sq)

        # Step 5: Sample from q(x_{\tau_{i - 1}} | x_{\tau_t}, x_0)
        # HINT: Use the reparameterization trick
        mu_tau_i = torch.sqrt(alpha_tau_isub1) * x_0 + coef_epsilon_tau_i * pred_noise
        z = torch.randn_like(img)
        img = mu_tau_i + sigma_tau_i_sq * z
