...
def get_optimizers_and_schedulers(gen, disc):
    ...
    # 100K iterations.
    ##################################################################
    scheduler_discriminator = torch.optim.lr_scheduler.LinearLR(optim_discriminator, start_factor= 1.0, end_factor= 0.0, total_iters= 5e5)
    scheduler_generator = torch.optim.lr_scheduler.LinearLR(optim_generator, start_factor= 1.0, end_factor= 0.0, total_iters= 1e5)
    ##################################################################
    #                          END OF YOUR CODE                      #
    ##################################################################
    return (
...
def train_model(
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        num_workers=0,       
        pin_memory=True,
    )#changed to num_workers to 0 from 4
    ...
    while iters < num_iterations:
        for train_batch in train_loader:
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                ...
                b_s = len(train_batch)
                generated = gen(b_s)
                discrim_real = disc(train_batch)  #should be 1
                discrim_fake = disc(generated)    ##should be 0
                ##################################################################
                #                          END OF YOUR CODE                      #
                ##################################################################
                ...
                # discriminator on it.
                ###################################################################
                ran = torch.rand(1).cuda()
                interp = ran * generated + (1 - ran) * train_batch
