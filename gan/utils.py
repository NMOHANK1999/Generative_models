...
def interpolate_latent_space(gen, path):
    ...
    ##################################################################
    lat_space = torch.zeros((100, 128)).cuda()
    x, y = torch.meshgrid(torch.linspace(-1, 1, 10), torch.linspace(-1, 1, 10))
    x = x.view(-1, 1)
    y = y.view(-1, 1) 
    lat_space[:, :2] = torch.cat((x, y), dim = 1)
    gen_images = gen.forward_given_samples(lat_space.cuda())
    torch.torchvision.utils.save_image(gen_images, path)
    ##################################################################
    #                          END OF YOUR CODE                      #
    ##################################################################
