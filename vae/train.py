...
def ae_loss(model, x):
    ...
    pred = model.decoder(out)

    loss= (loss_func(pred, x))/batch_size
...
#     var = torch.exp(0.5 * log_std)
#     epsilon = torch.randn_like(var).cuda()
#     z = mu + var * epsilon 
#     pred = model.decoder(z)
#     kl_loss = -0.5 * torch.sum(1 + log_std - mu.pow(2) - log_std.exp())
#     # epsilon = torch.rand(size = mu.shape).cuda()
#     # z = mu + torch.exp(log_std) * epsilon
#     # pred = model.decoder(z)
#     # #var = torch.exp(log_std)**2
#     # kl_loss = torch.mean(1/2 * torch.sum(-(torch.log(var) + 1) + var + mu**2, dim=1))
#     recon_loss = loss_fn(pred, x) / batch_size
...
def vae_loss(model, x, beta = 1):
    ...
    mu, log_std = model.encoder(x)
    z = mu + torch.exp(log_std) * torch.randn(size=mu.shape, device="cuda")
    out = model.decoder(z.cuda())

    B = x.shape[0]

    fn = nn.MSELoss(reduction='sum')
    recon_loss = fn(out, x) / B
    var = torch.exp(log_std)**2
    kl_loss = torch.mean(1/2 * torch.sum(-(torch.log(var) + 1)+var+mu**2, dim=1))
